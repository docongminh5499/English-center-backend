...
class StudentServiceImpl implements StudentServiceInterface {
  async getCoursesForTimetableByStudent(studentId: number): Promise<Course[]> {
    const studentCourse =
      await CourseRepository.findCourseForTimetableByStudent(studentId);
  ...
  async getCoursesByStudent(
    ...
    const [courseCount, courseList] = await Promise.all([
      ...
      CourseRepository.findCourseByStudent(
        pageable,
        sortable,
        selectable,
        queryable,
  ...
  async getCourseDetail(
    ...
    // course?.studentPaticipateCourses.forEach((value) =>
    //   console.log(value.student.user.id)
    // );
    if (
      course?.studentPaticipateCourses.filter(
        (value) => value.student.user.id === studentId
      ).length === 0
    )
      return null;
		// console.log(`Course Detail time execution ${-start.getTime() + (new Date()).getTime()}`);
    return course;
  }
  ...
  async getTotalCourseStudySession(courseSlug: string): Promise<number | null> {
    ...
    const totalStudySession = await StudySession.createQueryBuilder(
      "studySession"
    )
  ...
  async assessCourse(
    ...
    const course = await Course.createQueryBuilder("course")
      ...
      .andWhere("course.lockTime IS NULL OR course.lockTime > :now", {
        now: new Date(),
      })
      .getOne();
    ...
    if (
      !(
        Math.abs(expectedClosingDate.getTime() - now.getTime()) <
        14 * 24 * 60 * 60 * 1000
      )
    ) {
      return false;
    ...
    const studentParticipateCourse =
      await StudentParticipateCourse.createQueryBuilder()
        ...
        .getOne();
    if (studentParticipateCourse == null) return false;
  ...
  async getAttendance(
    studentId: number,
    courseSlug: string
  ): Promise<UserAttendStudySession[]> {
    // console.log("STUDENT ATTENDANCE SERVICE");
		// const start = new Date();
    const attendance = await StudySessionRepository.findStudySessionByStudent(
      studentId,
      courseSlug
    );
    // console.log(attendance);
		// console.log(`Attendance time execution ${-start.getTime() + (new Date()).getTime()}`);
    return attendance!;
  }
  ...
  async submitExercise(
    ...
    try {
      const student = await UserStudentRepository.findUserStudentById(
        studentId
      );
      const studentDoExercise = await StudentDoExercise.createQueryBuilder(
        "studentDoExercise"
      )
      ...
      const now = new Date();
      if (
        studentDoExercise.exercise.openTime.getTime() > now.getTime() ||
        now.getTime() > studentDoExercise.exercise.endTime.getTime()
      ) {
        return null;
      }
      let rightAnswer = 0;
      ...
      studentDoExercise.score = parseFloat(
        ((rightAnswer / answers.length) * 10).toPrecision(2)
      );
  ...
  async startDoExercise(
    ...
    try {
      ...
      const now = new Date();
      if (
        exercise.openTime.getTime() > now.getTime() ||
        now.getTime() > exercise.endTime.getTime()
      ) {
        return null;
      }
  ...
  async getStudentDoExercise(
    studentId: number,
    courseId: number
  ): Promise<StudentDoExercise[] | null> {
    try {
      const studentDoExercise = StudentDoExercise.createQueryBuilder(
        "studentDoExercise"
      )
        .setLock("pessimistic_read")
        .useTransaction(true)
  ...
  async getMakeupLessionCompatible(
    curriculumId: number,
    branchId: number,
    order: number,
    courseId: number
  ...
  > {
    const courses = await Course.createQueryBuilder("course")
      ...
      .leftJoinAndSelect(
        "course.studentPaticipateCourses",
    ...
    for (const course of courses) {
      ...
      const now = new Date();
      if (
        new Date(course.openingDate).getTime() - now.getTime() <
        14 * 24 * 60 * 60 * 1000
      )
        continue;
      const compatibleStudySession = await StudySession.createQueryBuilder(
        "studySession"
      )
        .setLock("pessimistic_read")
        .useTransaction(true)
        ...
        .take(1)
        .getOne();
      if (compatibleStudySession === null) continue;
      let numStudentWillAttend = course.studentPaticipateCourses.length;
      const numStudentLeave = await MakeUpLession.createQueryBuilder(
        "makeupLession"
      )
      ...
      const numStudentRegisterMakeup = await MakeUpLession.createQueryBuilder(
        "makeupLession"
      )
        .leftJoinAndSelect(
          "makeupLession.targetStudySession",
          "targetStudySession"
        )
        .where("targetStudySession.id = :ssid", {
          ssid: compatibleStudySession.id,
        })
        .getCount();

      numStudentWillAttend =
        numStudentWillAttend + numStudentRegisterMakeup - numStudentLeave;
  ...
  async registerMakeupLession(
    studentId: number,
    studySessionId: number,
    targetStudySessionId: number
  ): Promise<MakeUpLession | null> {
    const studySession = await StudySession.createQueryBuilder("studySession")
      .setLock("pessimistic_read")
      .useTransaction(true)
      .where("studySession.id = :studySessionId", {
        studySessionId: studySessionId,
      })
      .getOne();

    const targetStudySession = await StudySession.createQueryBuilder("ss")
      .setLock("pessimistic_read")
      .useTransaction(true)
    ...
    if (
      studySession === null ||
      targetStudySession === null ||
      userStudent === null
    ) {
      return null;
    }

    let numStudentWillAttend =
      targetStudySession.course.studentPaticipateCourses.length;
    const numStudentLeave = await MakeUpLession.createQueryBuilder(
      "makeupLession"
    )
      .leftJoinAndSelect("makeupLession.studySession", "studySession")
    ...
    const savedMakeupLession = await MakeUpLession.createQueryBuilder(
      "makeupLession"
    )
      .setLock("pessimistic_read")
      .useTransaction(true)
      ...
      .andWhere("targetStudySession.id = :targetStudySessionId", {
  ...
  async getMakeupLession(studentId: number): Promise<MakeUpLession[] | null> {
    ...
    const makeupLession = await MakeUpLession.createQueryBuilder(
      "makeupLession"
    )
      .setLock("pessimistic_read")
      .useTransaction(true)
  ...
  async deleteMakeupLession(
    studentId: number,
    studySessionId: number,
    targetStudySessionId: number
  ): Promise<boolean> {
    if (!studentId || !studySessionId || !targetStudySessionId) return false;

    await MakeUpLession.createQueryBuilder("makeupLession")
      .setLock("pessimistic_read")
      .useTransaction(true)
      .leftJoinAndSelect("makeupLession.student", "student")
      .leftJoinAndSelect("student.user", "user")
  ...
  async getPersonalInformation(studentId: number): Promise<UserStudent | null> {
    const userStudent = await UserStudent.createQueryBuilder("userStudent")
  ...
  async getParentList(searchValue: string): Promise<UserParent[] | null> {
    ...
    const userParentList = await UserParent.createQueryBuilder("userParent")
  ...
  async addParent(
    studentId: number,
    parentId: number
  ): Promise<UserParent | null> {
    const userParent = await UserParent.createQueryBuilder("userParent")
      .leftJoinAndSelect("userParent.user", "user")
  ...
  async modifyPersonalInformation(
    userId: number,
    userStudent: UserStudent,
    avatarFile?: FileDto | null
  ): Promise<CredentialDto | null> {
    const persistenceUserStudent = await UserStudent.createQueryBuilder(
      "userStudent"
    )
      .leftJoinAndSelect("userStudent.user", "user")
    ...
    try {
      ...
      persistenceUserStudent.user.dateOfBirth = moment(
        userStudent.user.dateOfBirth
      ).toDate();
      ...
      if (avatarFile && avatarFile.filename)
        persistenceUserStudent.user.avatar =
          AVATAR_DESTINATION_SRC + avatarFile.filename;

      const savedUser = await queryRunner.manager.save(
        persistenceUserStudent.user
      );
      await queryRunner.manager.upsert(UserStudent, persistenceUserStudent, {
        conflictPaths: [],
  ...
  async getPaymentHistory(
    ...
    limit: number,
    skip: number
  ): Promise<{ fee: Fee[]; total: number } | null> {
    const userStudent = await UserStudentRepository.findUserStudentById(
      studentId
    );
    if (userStudent === null) {
      return null;
    }
    const paymentHistory = await Fee.createQueryBuilder("fee")
    ...
    let total = await Fee.createQueryBuilder("fee")
      .leftJoinAndSelect("fee.transCode", "transCode")
      .leftJoinAndSelect("fee.course", "course")
      .leftJoinAndSelect("fee.userStudent", "userStudent")
      .leftJoinAndSelect("userStudent.user", "user")
    ...
    const arrStudentCourse = await StudentParticipateCourse.createQueryBuilder(
    ...
    for (const studentParticipateCourse of arrStudentCourse) {
      ...
      const courseClosingDate = new Date(
        studentParticipateCourse.course.expectedClosingDate
      );
      ...
      while (billingDate.getTime() < now.getTime()) {
        ...
        if (billingDate.getTime() < now.getTime()) {
          ...
          tranCode.transCode = "-";
          tranCode.content = `${studentParticipateCourse.course.name} (Tháng ${
    ...
    if (arrUnpaidFee.length > skip) {
      ...
      const addLength =
        arrUnpaidFee.length > limit ? limit : arrUnpaidFee.length;

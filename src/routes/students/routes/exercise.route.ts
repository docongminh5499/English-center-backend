...
router.post("/submit-exercise", async (req: any, res: any, next: any) => {
  try {
      ...
      const studentDoExercise = await StudentService.submitExercise(req.user.userId, req.body.doingId, req.body.answers);
...
router.post("/start-do-exercise", async (req: any, res: any, next: any) => {
  try {
      // console.log("************************************************");
      // console.log(req.body);
      const startDoExercise = await StudentService.startDoExercise(req.user.userId, req.body.exerciseId);
      if(startDoExercise === null){
        throw new Error("Lỗi hệ thống");
      }
      return res.status(200).json(startDoExercise.id);
    } catch (err) {
      console.log(err);
      next(err);
    }

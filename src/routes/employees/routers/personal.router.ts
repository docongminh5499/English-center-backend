...
router.post("/modify-personal-information", upload.single('avatar'), async (req: any, res: any, next: any) => {
  try {
    const result = await EmployeeService
      .modifyPersonalInformation(req.user.userId, JSON.parse(req.body.userEmployee), req.file);
...
router.post("/get-salaries", async (req: any, res: any, next: any) => {
  try {
    ...
    const result = await EmployeeService.getPersonalSalaries(req.user.userId, pageableDto, req.body.fromDate, req.body.toDate);
